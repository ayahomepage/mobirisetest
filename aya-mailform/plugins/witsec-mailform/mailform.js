...
function listen(selector, event, func) {
	...
	for (var i = 0; i < el.length; i++){
		console.log('el[i].name:'+el[i].name)
		console.log('el[i].method:'+el[i].method)
		console.log('el[i].target:'+el[i].target)
		console.log('el[i].action:'+el[i].action)
		el[i].addEventListener(event, func);
	}

	$.ajax({
		url: 'https://aya-mobirisetest.microcms.io/api/v1/blog',
		type: 'GET',
		headers: {
		  'Content-Type': 'application/json',
		  'X-MICROCMS-API-KEY': 'e1144ab8542f44a7b7e442b3f191f3e9c9fc'
		},
	})
	.done(function(data) {
